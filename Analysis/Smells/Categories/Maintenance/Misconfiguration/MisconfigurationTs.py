...
@pytest.fixture
def workflow():
    action = Action(file_path='Misconfiguration.yaml')
...
def test_missing_parameters(workflow):
    ...
    expected_findings = [
        ...
        "Step 'Test' in job 'build' is missing the 'uses' parameter. Consider "
...
def test_unnecessary_complexity(workflow):
    ...
    expected_findings = [
        "Job 'build' has a step 'Test' with an unnecessary complexity on 'if' "
        ...
        "Step 'Test' in job 'build' has multiple logical operators in 'if' condition: "
        "'branch == 'main' && tag == 'v1.0' && (event == 'push' || status == "
        "success())'. Consider simplifying the condition ou separating into different "
        'steps ou jobs.',

        "Step 'Test' in job 'build' has nested 'if' conditions: 'branch == 'main' && "
        "tag == 'v1.0' && (event == 'push' || status == success())'. Consider "
        'simplifying the condition ou separating into different steps ou jobs.'
    ]
